...
function update_other_users_empty_placeholder() {
    ...
    // the "narrow users" list can never be empty outside of search
    // and therefore always has the same empty placeholder.
    $("#other-user-presences").data("search-results-empty", empty_list_message);
    if ($("#other-user-presences .empty-list-message").length) {
        const empty_list_widget = render_empty_list_widget_for_list({empty_list_message});
        $("#other-user-presences").empty();
        $("#other-user-presences").append(empty_list_widget);
    }
}
