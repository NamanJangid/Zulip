...
test("direct_message_update_dom_counts", () => {
    ...
    buddy_list_add_narrow_user(pm_key, $li);
...
test("handlers", ({override, mock_template}) => {
    ...
    override(buddy_list, "fill_screen_with_content", () => {
        ...
        buddy_list_add_narrow_user(me.user_id, $me_li);
        buddy_list_add_narrow_user(alice.user_id, $alice_li);
        buddy_list_add_narrow_user(fred.user_id, $fred_li);
    });
...
test("insert_fred_then_alice_then_rename, both as narrow users", ({override, mock_template}) => {
    mock_template("presence_row.hbs", true, (_data, html) => html);

    add_sub_and_set_as_current_narrow(rome_sub);
    peer_data.set_subscribers(rome_sub.stream_id, [alice.user_id, fred.user_id]);

    let narrow_users_appended_html;
    override(buddy_list.$narrow_users_container, "append", (html) => {
        narrow_users_appended_html = html;
    });
    override(padded_widget, "update_padding", () => {});
    buddy_list_add_narrow_user(alice.user_id, $alice_stub);
    buddy_list_add_narrow_user(fred.user_id, $fred_stub);
    ...
    activity_ui.redraw_user(fred.user_id);
    assert.ok(narrow_users_appended_html.indexOf('data-user-id="2"') > 0);
    assert.ok(narrow_users_appended_html.indexOf("user_circle_green") > 0);
...
test("initialize", ({override}) => {
    ...
    function clear() {
        $.clear_all_elements();
        buddy_list.$narrow_users_container = $("#narrow-user-presences");
